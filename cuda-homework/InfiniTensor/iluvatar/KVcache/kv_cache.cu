...
#define KV_BULK_THREADS 256
#endif

// 天垓的一个 warp 有 64 个 lane。这里必须跟着硬件来，否则一个 block 里
// “第几个 warp”和“warp 内第几个线程”都会算错，decode 小路径会直接串 head。
constexpr int kWarpSize = 64;
constexpr int kDecodeWarps = KV_DECODE_WARPS;
constexpr int kBulkThreads = KV_BULK_THREADS;
constexpr int kMaxBlocks = 4096;
static_assert(kDecodeWarps > 0 && kDecodeWarps <= 16,
              "KV_DECODE_WARPS must be in [1, 16]");
static_assert(kBulkThreads > 0 && kBulkThreads <= 1024
                  && kBulkThreads % kWarpSize == 0,
              "KV_BULK_THREADS must be a 64-lane-warp-aligned block size");
...
cudaError_t launch_update_pack(
  ...
  if (new_tokens > 1) {
    ...
    // prefill 数据够多，直接铺平给一批线程搬。线程数留成编译参数，方便在
    // 新型号上跑几组小实验，不必为了调参改 kernel 本身。
    const int threads = kBulkThreads;
