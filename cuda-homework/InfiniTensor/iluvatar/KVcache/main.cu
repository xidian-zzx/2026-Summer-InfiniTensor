...
bool same_bits<half>(half lhs, half rhs) {
  // CoreX 把 __half_as_ushort 做成了 device-only，host 测试端直接拷贝这
  // 16 个 bit 更省心，也正好符合 KV Cache 要求逐 bit 原样搬运的语义。
  unsigned short lhs_bits = 0;
  unsigned short rhs_bits = 0;
  static_assert(sizeof(half) == sizeof(lhs_bits), "unexpected half size");
