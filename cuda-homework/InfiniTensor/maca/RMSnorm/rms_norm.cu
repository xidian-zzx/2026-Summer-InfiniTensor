...
// C500 的硬件 wavefront 是 64 线程；cu-bridge 会提供 32 线程的 CUDA 逻辑
// warp。这里按逻辑 warp 写归约，既能正确使用 shuffle，也方便和 CUDA 版对照。
...
#pragma unroll
  ...
  const float inv_rms = __shfl_sync(
      0xffffffffu, rsqrtf(square_sum / static_cast<float>(hidden_dim) + eps), 0);
...
cudaError_t launch_block_path(const T* input, const T* weight, T* output,
  ...
  // C500 上 float 缓存一整行很划算；half 转成 float 再缓存会把数据膨胀
  // 两倍，还会占掉不少共享内存。实测 half 直接读两遍更快，所以默认分流。
...
cudaError_t launch_impl(const T* input, const T* weight, T* output, int rows,
  ...
  //
  // float4 和 half8 都是 16 字节。384、640、896 这几个 half 尺寸无法让
  // 32 个线程平均分配 half8，于是改用 half4，仍然保持向量化读取。
  if constexpr (std::is_same<T, float>::value) {
    switch (hidden_dim) {
      case 128:  return launch_warp_path<T, 4, 1>(input, weight, output, rows, hidden_dim, eps, stream);
      case 256:  return launch_warp_path<T, 4, 2>(input, weight, output, rows, hidden_dim, eps, stream);
      case 384:  return launch_warp_path<T, 4, 3>(input, weight, output, rows, hidden_dim, eps, stream);
      case 512:  return launch_warp_path<T, 4, 4>(input, weight, output, rows, hidden_dim, eps, stream);
      case 640:  return launch_warp_path<T, 4, 5>(input, weight, output, rows, hidden_dim, eps, stream);
      case 768:  return launch_warp_path<T, 4, 6>(input, weight, output, rows, hidden_dim, eps, stream);
      case 896:  return launch_warp_path<T, 4, 7>(input, weight, output, rows, hidden_dim, eps, stream);
      case 1024: return launch_warp_path<T, 4, 8>(input, weight, output, rows, hidden_dim, eps, stream);
      default: break;
    }
    ...
    switch (hidden_dim) {
      case 128:  return launch_warp_path<T, 4, 1>(input, weight, output, rows, hidden_dim, eps, stream);
      case 256:  return launch_warp_path<T, 8, 1>(input, weight, output, rows, hidden_dim, eps, stream);
      case 384:  return launch_warp_path<T, 4, 3>(input, weight, output, rows, hidden_dim, eps, stream);
      case 512:  return launch_warp_path<T, 8, 2>(input, weight, output, rows, hidden_dim, eps, stream);
      case 640:  return launch_warp_path<T, 4, 5>(input, weight, output, rows, hidden_dim, eps, stream);
      case 768:  return launch_warp_path<T, 8, 3>(input, weight, output, rows, hidden_dim, eps, stream);
      case 896:  return launch_warp_path<T, 4, 7>(input, weight, output, rows, hidden_dim, eps, stream);
      case 1024: return launch_warp_path<T, 8, 4>(input, weight, output, rows, hidden_dim, eps, stream);
      default: break;
    }
